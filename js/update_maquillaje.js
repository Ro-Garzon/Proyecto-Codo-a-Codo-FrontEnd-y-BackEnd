...
document.getElementById("txtDesc").value = decodeURIComponent(parts[3][1])
...
function modificar() {
    ...
    let url = "http://romina.pythonanywhere.com/maquillajes/"+id
    var options = {
        body: JSON.stringify(maquillaje),
        method: 'PUT',
    ...
    fetch(url, options)
        .then(function () {
            ...
            window.location.href = "./productos_maquillaje.html"
        })
        .catch(err => {
            //this.errored = true
            console.error(err);
